...
void validateOceanState(const OceanState *State, const AuxiliaryState *AuxState,
   ...
   // -------------------------------------------------------------------------
   {
      ...
      if (NaNs > 0) {
         LOG_CRITICAL("StateValidation: LayerThickness contains {} NaN value(s)",
                      NaNs);
         AnyFailure = true;
      }
      if (OOB > 0) {
         LOG_CRITICAL(
             "StateValidation: LayerThickness has {} value(s) outside "
   ...
   // -------------------------------------------------------------------------
   // KineticEnergyCell: valid range (-inf, 10]
   // -------------------------------------------------------------------------
   {
      ...
      if (OOB > 0) {
         LOG_CRITICAL(
             "StateValidation: KineticEnergyCell has {} value(s) exceeding "
   ...
   if (Tracers::IndxTemp != Tracers::IndxInvalid) {
      ...
      auto [NaNs, OOB] =
          checkTracerArray(AllTracers, Tracers::IndxTemp, State->NCellsOwned,
                           State->NVertLayers, static_cast<Real>(-10.0),
                           static_cast<Real>(50.0));
      ...
      if (OOB > 0) {
         LOG_CRITICAL(
             "StateValidation: Temperature has {} value(s) outside "
             "valid range [-10, 50]",
             OOB);
         AnyFailure = true;
      }
   ...
   if (Tracers::IndxSalt != Tracers::IndxInvalid) {
      Array3DReal AllTracers = Tracers::getAll(TimeLevel);
      auto [NaNs, OOB] =
          checkTracerArray(AllTracers, Tracers::IndxSalt, State->NCellsOwned,
                           State->NVertLayers, static_cast<Real>(-2.0),
      ...
      if (OOB > 0) {
         LOG_CRITICAL(
             "StateValidation: Salinity has {} value(s) outside "
             "valid range [-2, 60]",
             OOB);
         AnyFailure = true;
      }
   ...
   if (AnyFailure) {
      abortWithMessage(
          "StateValidation: Ocean state validation failed. "
